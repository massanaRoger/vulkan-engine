...
std::optional<std::shared_ptr<LoadedGLTF>> loadGltf(std::string_view filePath)
{
	...
	fmt::print(fmt::runtime("Loading GLTF: {}"), filePath);
	...
	file.descriptorPool.init(renderer.device, gltf.materials.size(), sizes);
	...
	for (fastgltf::Sampler& sampler : gltf.samplers) {
		...
		vkCreateSampler(renderer.device, &sampl, nullptr, &newSampler);
	...
	file.materialDataBuffer = renderer.create_buffer(sizeof(GLTFMetallic_Roughness::MaterialConstants) * gltf.materials.size(), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VMA_MEMORY_USAGE_CPU_TO_GPU, VK_SHARING_MODE_EXCLUSIVE);
	int data_index = 0;
